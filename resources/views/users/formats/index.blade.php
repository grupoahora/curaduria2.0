...
@section('content')
    ...
    <div class="card-header">
        <a class="btn btn-secondary btn-sm float-right" href="{{ route('admin.formats.create') }}">Nuevo Formato</a>
        <h1>Lista de Formatos</h1>

    </div>
    <div class="card-body">
        <table id="TableFormats" class="table table-striped table-responsive-md">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>Nombre</th>
            ...
            <tbody>
                @foreach ($formats as $format)
                    <tr>
                        <td>{{ $format->id }}</td>
                        <td>
                            <a href="{{ route('admin.formats.show', $format) }}">{{ $format->name }}</a>
                        </td>
                        <td>{{ $format->created_at }}</td>
                        <td>{{ $format->updated_at }}</td>
                        <td width="10px">
                            <a href="{{ route('admin.formats.edit', $format) }}" class="btn btn-primary btn-sm">Editar</a>
                        </td>
                        <td width="10px">
                            <form action="{{ route('admin.formats.destroy', $format) }}" method="POST">
                                @csrf
                                @method('delete')
